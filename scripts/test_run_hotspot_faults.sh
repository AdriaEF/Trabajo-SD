...
# Temporary multi-machine smoke test for Part 6 and Part 7 only.
# Keeps the same calling convention as test_run_server.sh.
#
# Usage:
#   bash test_run_hotspot_faults.sh <server_ip> <remote_servers> [total_workers] [rabbitmq_ip] [rabbitmq_user] [rabbitmq_pass]
#
# Example:
#   bash test_run_hotspot_faults.sh 192.168.247.247 "192.168.247.112:8001 192.168.247.112:8002" 4 192.168.247.247 admin test

if [[ $# -lt 2 ]]; then
    echo "Usage: bash test_run_hotspot_faults.sh <server_ip> <remote_servers> [total_workers] [rabbitmq_ip] [rabbitmq_user] [rabbitmq_pass]" >&2
    exit 1
fi
