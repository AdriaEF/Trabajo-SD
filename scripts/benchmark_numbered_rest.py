...
def send_buy(base_url: str, timeout: float, client_id: str, seat_id: int, request_id: str) -> str:
    payload = json.dumps(
        {"client_id": client_id, "seat_id": seat_id, "request_id": request_id}
    ).encode("utf-8")
    req = request.Request(
...
def parse_benchmark(path: str) -> list[tuple[str, int, str]]:
    ...
    with open(path, "r", encoding="utf-8") as f:
        for raw_line in f:
            ...
            parts = line.split()
            # Expected format: BUY <client_id> <seat_id> <request_id>
...
def run_benchmark(
    ops: list[tuple[str, int, str]], base_url: str, concurrency: int, timeout: float
) -> Counter:
    ...
    with ThreadPoolExecutor(max_workers=concurrency) as executor:
        futures = [
            executor.submit(send_buy, base_url, timeout, client_id, seat_id, request_id)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run numbered REST benchmark")
    parser.add_argument("--file", required=True, help="Path to benchmark_numbered.txt")
    ...
    print("=== Benchmark Results (Numbered REST) ===")
